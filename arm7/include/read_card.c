...
static void initKey1Encryption (u8* cmdData, int iCardDevice) {
	...
	key1data.llll = rand() & 0x0000ffff;
	key1data.mmm = rand() & 0x00000fff;
	key1data.nnn = rand() & 0x00000fff;
...
/*static void cardReadBlock(u32 src, u8* dest) {
	...
	cardParamCommand (CARD_CMD_DATA_READ, src, portFlags | CARD_ACTIVATE | CARD_nRESET | CARD_BLK_SIZE(1), tmp_dest, 0x200 / sizeof(uint32_t));
	tonccpy(dest, tmp_dest, 0x200);
}

void cardRead(u32 src, u8* dest, size_t size) {
	if(size == 0)return;

	// Make it sure it's properly aligned
	cardReadBlock(src, dest);
	size_t offset_read_size = ((src + 0x200) & (~0x1FF)) - src;
	if(size <= offset_read_size)return;
	size -= offset_read_size;
	src += offset_read_size;
	dest += offset_read_size;

	for(u32 i = 0; i < ((size + 0x200 - 1) / 0x200); i++)cardReadBlock(src + (i * 0x200), dest + (i * 0x200));
}*/
